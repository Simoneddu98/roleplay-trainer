...
import ScenarioVideo, { SCENARIO_CONFIG } from '@/components/remotion/ScenarioVideo';
import { useAppStore } from '@/store/useStore';
import {
  ArrowLeft, Play, Pause, RotateCcw, Zap, CheckCircle2, Clapperboard,
} from 'lucide-react';
...
export default function SimulationPage() {
  ...
  useEffect(() => {
    ...
    const handleFrameUpdate = (e: { detail: { frame: number } }) => {
      ...
      if (simState === 'playing' && branch === 'none' && frame >= SCENARIO_CONFIG.pauseAtFrame) {
        player.pause();
        setSimState('paused-for-choice');
      }
      if (simState === 'branch-playing' && frame >= SCENARIO_CONFIG.durationInFrames - 5) {
  ...
  }, [simState, branch, hasAwarded, dispatch]);

  const handleBranchChoice = useCallback((chosen: 'A' | 'B') => {
    setBranch(chosen);
    setSimState('branch-playing');
    // Small delay to let React re-render with new branch prop
    setTimeout(() => {
      const player = playerRef.current;
      if (player) {
        player.seekTo(SCENARIO_CONFIG.branchA.goToFrame);
        player.play();
      }
    }, 100);
  }, []);
        ...
        <div className="relative bg-black rounded-2xl overflow-hidden shadow-xl">
          <div className="aspect-video">
            <Player
              ...
              inputProps={{ branch }}
              durationInFrames={SCENARIO_CONFIG.durationInFrames}
              fps={SCENARIO_CONFIG.fps}
              compositionWidth={SCENARIO_CONFIG.width}
              compositionHeight={SCENARIO_CONFIG.height}
              ...
              <div className="bg-white rounded-2xl p-8 mx-4 max-w-lg w-full shadow-2xl text-center space-y-6">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-2">
                    Il cliente ha sollevato un&apos;obiezione sul prezzo. Come rispondi?
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-900 text-sm">{SCENARIO_CONFIG.branchA.label}</p>
                        <p className="text-xs text-slate-500 mt-0.5">Presenti dati concreti per giustificare il prezzo</p>
                      </div>
                    </div>
                  </button>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-900 text-sm">{SCENARIO_CONFIG.branchB.label}</p>
                        <p className="text-xs text-slate-500 mt-0.5">Enfatizzi il valore aggiunto e il supporto incluso</p>
                      </div>
                    </div>
                  </button>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    ...
                    <span className="font-semibold text-indigo-600">
                      {branch === 'A' ? 'Risposta Strategica' : 'Risposta Empatica'}
                    </span>
                  </p>
                </div>
