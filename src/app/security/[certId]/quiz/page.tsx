...
import { getSecurityQuestions } from '@/data/security-trivia';
import { TriviaQuestion } from '@/data/trivia';
import {
  ArrowLeft, Trophy, RotateCcw, Zap, CheckCircle, XCircle, Brain, Shield,
} from 'lucide-react';
...
export default function SecurityQuizPage() {
  ...
  const certName = area?.title ?? 'Certificazione';

  const loadQuestions = useCallback(
    () => getSecurityQuestions(certId).map(prepareQuestion),
    [certId],
  );
  ...
  if (gameState === 'finished') {
    ...
    return (
      <div className="min-h-screen bg-[#06060a] flex items-center justify-center px-4">
        <div className="bg-white/[0.03] rounded-2xl border border-white/[0.06] shadow-2xl shadow-amber-500/5 p-8 text-center max-w-md w-full space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Quiz completato!</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={restartGame}
              className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-amber-600 to-orange-600 text-white rounded-xl font-medium hover:from-amber-500 hover:to-orange-500 shadow-lg shadow-amber-500/25 transition-all cursor-pointer"
            >
              <RotateCcw className="w-4 h-4" />
              Rigioca
            </button>
            <button
              onClick={() => router.push(`/security/${certId}`)}
    ...
    <div className="min-h-screen bg-[#06060a]">
      ...
      <div className="sticky top-0 z-50 bg-[#06060a]/80 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <button
            onClick={() => router.push(`/security/${certId}`)}
            className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-amber-400 transition-colors cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" />
            Esci
          </button>
        ...
        <div className="bg-white/[0.03] rounded-2xl border border-white/[0.06] overflow-hidden">
          ...
          <div className="p-6 space-y-3">
            {currentQ.answers.map((answer, i) => {
              let btnClass = 'border-white/[0.08] bg-white/[0.02] hover:border-amber-500/40 hover:bg-amber-500/10 text-slate-300';
          ...
          {gameState === 'answered' && (
            ...
            >
              ...
              <button
                onClick={handleNext}
                className="px-5 py-2 bg-gradient-to-r from-amber-600 to-orange-600 text-white rounded-lg text-sm font-medium hover:from-amber-500 hover:to-orange-500 shadow-lg shadow-amber-500/25 transition-all cursor-pointer"
              >
