...
import { Send, Bot, User, ArrowLeft, Loader2, Shield } from 'lucide-react';
...
export default function EfisioChatbotPage() {
    ...
    <div className="flex flex-col h-screen bg-[#06060a]">
      ...
      <div className="shrink-0 border-b border-white/[0.06] bg-white/[0.02] backdrop-blur-xl">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="w-9 h-9 bg-gradient-to-br from-violet-600 to-purple-600 rounded-lg flex items-center justify-center shadow-lg shadow-violet-500/20 text-white font-bold text-sm">
            E
          </div>

          <div className="flex-1">
            <h1 className="text-sm font-semibold text-white">Efisio</h1>
            <p className="text-xs text-slate-500">
              {isStreaming ? 'Sta scrivendo...' : 'Tutor Marketing & Vendita'}
            </p>
          </div>

          <Link
            href="/chatbot/gavino"
            className="flex items-center gap-1.5 text-xs text-amber-400 hover:text-amber-300 bg-amber-500/10 hover:bg-amber-500/15 border border-amber-500/20 rounded-full px-3 py-1.5 transition-all"
          >
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-600/20 to-purple-600/20 border border-violet-500/10 flex items-center justify-center mb-4">
                <Bot className="w-8 h-8 text-violet-400" />
              </div>
              <h2 className="text-lg font-semibold text-white mb-2">Eja! Ciao, sono Efisio</h2>
              <p className="text-sm text-slate-500 max-w-sm mb-6">
                Il tuo tutor AI per Marketing e Tecniche di Vendita. Scrivimi qualsiasi domanda, Ajò!
              </p>
              <div className="flex flex-wrap justify-center gap-2 max-w-sm">
                {EFISIO_CHIPS.map((chip) => (
                  <button
                    key={chip}
                    onClick={() => handleSend(chip)}
                    className="text-xs px-3.5 py-2 rounded-full border border-violet-500/30 bg-violet-500/10 text-violet-300
                      hover:bg-violet-500/20 transition-all cursor-pointer font-medium"
                  >
                    {chip}
                  </button>
                ))}
